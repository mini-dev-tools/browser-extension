...
export const navigationSections: NavigationSection[] = [
  {
    label: 'Generate',
    description: 'Create and generate content',
    icon: Zap,
    items: [
      {
        label: 'Lorem Ipsum',
      ...
      },
      {
        ...
        path: '/swagger-viewer',
        description: 'View OpenAPI/Swagger docs offline',
        icon: Book
      }
    ]
  },
    ...
    items: [
      {
        label: 'Color Picker',
        path: '/color-picker',
        description: 'Pick and analyze colors',
        icon: Pipette
      },
      {
        label: 'Color Shades',
  ...
  {
    label: 'Browser Tools',
    description: 'Browser and extension utilities',
    icon: Monitor,
    items: [
      {
        label: 'Window Resizer',
        path: '/window-resizer',
...
export const getFilteredNavigationSections = (isChromeExtension: boolean): NavigationSection[] => {
  return navigationSections.map(section => ({
    ...section,
    items: section.items.filter(item => 
      !item.chromeExtensionOnly || isChromeExtension
    )
  })).filter(section => section.items.length > 0)
}

export const getFilteredMoreItems = (isChromeExtension: boolean): NavigationItem[] => {
  return moreNavigationItems.filter(item => 
    !item.chromeExtensionOnly || isChromeExtension
  )
}

// Helper function to check if running in Chrome extension
export const isChromeExtension = (): boolean => {
  return !!(window.chrome && chrome.runtime && chrome.runtime.id)
}

// Icon SVG mapping for compatibility with Index.vue
export const iconSvgMap: Record<string, string> = {
  ...
  Scissors: '<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M9 9V4.5M9 9H4.5M9 9L3.75 3.75M9 15v4.5M9 15H4.5M9 15l-5.25 5.25M15 9h4.5M15 9V4.5M15 9l5.25-5.25M15 15h4.5M15 15v4.5m0-4.5l5.25 5.25" /></svg>',
  GitCompare: '<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M7.5 7.5h-.75A2.25 2.25 0 004.5 9.75v7.5a2.25 2.25 0 002.25 2.25h7.5a2.25 2.25 0 002.25-2.25v-7.5a2.25 2.25 0 00-2.25-2.25h-.75m-6 3.75l3 3 6-6" /></svg>',
  Pipette: '<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M15 11.25l1.5 1.5.75-.75V8.758l2.276-.61a3 3 0 10-3.675-3.675l-.61 2.277H12l-.75.75 1.5 1.5M15 11.25l-8.47 8.47c-.34.34-.8.53-1.28.53s-.94.19-1.28.53l-.97.97-.75-.75.97-.97c.34-.34.53-.8.53-1.28s.19-.94.53-1.28L12.75 9M15 11.25L12.75 9" /></svg>',
  ...
  Blend: '<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M9.53 16.122a3 3 0 00-5.78 1.128 2.25 2.25 0 01-2.4 2.245 4.5 4.5 0 008.4-2.245c0-.399-.078-.78-.22-1.128zm0 0a15.998 15.998 0 003.388-1.62m-5.043-.025a15.994 15.994 0 011.622-3.395m3.42 3.42a15.995 15.995 0 004.764-4.648l3.876-5.814a1.151 1.151 0 00-1.597-1.597L14.146 6.32a15.996 15.996 0 00-4.649 4.763m3.42 3.42a6.776 6.776 0 00-3.42-3.42" /></svg>',
  ...
  Syringe: '<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M6.75 7.5l3 2.25-3 2.25m4.5 0h3m-9 8.25h13.5A2.25 2.25 0 0021 18V6a2.25 2.25 0 00-2.25-2.25H5.25A2.25 2.25 0 003 6v12a2.25 2.25 0 002.25 2.25z" /></svg>',
  Shield: '<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M18.364 18.364A9 9 0 005.636 5.636m12.728 12.728A9 9 0 015.636 5.636m12.728 12.728L5.636 5.636" /></svg>',
...
export const getIconSvg = (iconComponent: any): string => {
  const iconName = iconComponent?.name || iconComponent?.toString?.() || ''
  return iconSvgMap[iconName] || iconSvgMap.Type // fallback to Type icon
}